...
export default function CustomerAssignment() {
  ...
  const handleAssign = async (customerId, officerId) => {
    try {
      await api.put(`/users/${customerId}/assign-officer`, { officer_id: officerId || null })
  ...
  const handleBulkAssign = async () => {
    ...
    try {
      await api.post('/users/bulk-assign-officer', {
  ...
  const handleSyncFromLoans = async () => {
    try {
      const res = await api.post('/users/sync-officers-from-loans')
  ...
  const getOfficerName = (officerId) => {
    ...
    return officer ? `${officer.first_name} ${officer.last_name}` : 'Unassigned'
  }
  ...
  const filteredCustomers = customers.filter(customer => {
    ...
    const matchesOfficer = !filterOfficer || 
      (filterOfficer === 'unassigned' ? !customer.assigned_officer_id : customer.assigned_officer_id === parseInt(filterOfficer))
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow border dark:border-gray-700 p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold dark:text-white">{customers.length - unassignedCount}</p>
              </div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow border dark:border-gray-700 p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold dark:text-white">{unassignedCount}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border dark:border-gray-700 p-4 flex gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <select
            value={filterOfficer}
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-300 uppercase">Customer</th>
                <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-300 uppercase">Email</th>
                <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-300 uppercase">Assigned Officer</th>
                <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-300 uppercase">Change Officer</th>
              </tr>
            </thead>
